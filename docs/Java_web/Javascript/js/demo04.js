...
function delFruit(){
     if(event && event.srcElement && event.srcElement.tagName=="IMG"){
      //弹出一个对话框，如果选择确定则继续（alert只能确定，confirm可以取消）
          if(window.confirm()){
               var img=event.srcElement;
               var tr=img.parentElement.parentElement;
               //tr 有一个属性可以获取rowindex    table还有一个方法可以直接删除一行，但是需要行索引
               //先获取表格的名字
               var fruit_tb=document.getElementById("fruit_tb1");
               fruit_tb.deleteRow(tr.rowsIndex);
          }
     }
}
//编辑单价
     ...
     if(event && event.srcElement && event.srcElement.tagName=="TD"){
          ...
          if(priceTD.firstChild && priceTD.firstChild.nodeType==3){
               ...
               if(input.tagName=="INPUT"){
                    ...
                    //5.在输入框上绑定键盘按下的事件，需要保证用户输入的是数字
                    input.onclickdown=checkinput;
